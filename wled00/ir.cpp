...
{
  if ((code == 0xFFFFFFFF || code > 0xFFFFF) && (irTimesRepeated < 5) && (millis() - lastValidCodeTime < 2500)) {
  ...
  if (irEnabled == 8) { // any remote configurable with ir.json file
    bool ok = decodeIRJson(code);
    if (ok)
    {
      lastValidCodeTime = millis();
      irTimesRepeated = 0;
...
*/
bool decodeIRJson(uint32_t code)
{
  char objKey[10];
  String cmdStr;
  JsonObject fdo;
...
#else
  if (!requestJSONBufferLock(13)) return false;
#endif
