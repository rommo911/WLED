...
void PIRsensorSwitch::publishMqtt(bool switchOn)
{
  ...
  if (WLED_MQTT_CONNECTED)
  {
    for (int i = 0; i < PIR_SENSOR_MAX_SENSORS; i++)
    {
      ...
      mqtt->publish(buf, 0, false, sensorPinState[i] ? "on" : "off");
...
#ifdef ESP8266
      pinMode(PIRsensorPin[i], PIRsensorPin[i] == 16 ? INPUT_PULLDOWN_16 : INPUT); // ESP8266 has INPUT_PULLDOWN on GPIO16 only
#else
...
void PIRsensorSwitch::loop()
{
  ...
  if (!enabled || millis() - lastLoop < 200)
    return;
  lastLoop = millis();
...
bool PIRsensorSwitch::readFromConfig(JsonObject &root)
{
  ...
  else
  {
    for (int i = 0; i < PIR_SENSOR_MAX_SENSORS; i++)
      if (oldPin[i] >= 0)
        pinManager.deallocatePin(oldPin[i], PinOwner::UM_PIR);
    setup();
